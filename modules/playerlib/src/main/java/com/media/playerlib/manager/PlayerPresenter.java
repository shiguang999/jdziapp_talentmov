...
public class PlayerPresenter {
    ...
    public void setAuthCode(int authCode) {
        ...
        if (mAssist != null) {
            mAssist.getReceiverGroup().getGroupValue().putInt(DataInter.Key.AUTH_CODE, authCode);
        }
    }
    ...
    public void initView(Context context, final FrameLayout container, FrameLayout fullContainer, int authCode) {
        ...
        changeMode(false);
        receiverGroup.getGroupValue().putInt(DataInter.Key.AUTH_CODE, authCode);
        initDlans(context);
    }
...
//            dlanPresenter.showDialogTip(context, currentUrl, currentTitle, mAssist.getDuration());

            DlanListPop dlanListPop = new DlanListPop(context, currentUrl, currentTitle);
    ...
    public void resume() {
        ...
        if (authCode <= 2 && mAssist.getCurrentPosition() > 10 * 60 * 1000) {
            pause();
            return;
        }
...
//        dispatcher.dispatchReceiverEvent(DataInter.Event.RESTART_PLAY, null);
    ...
    public void switchPlayFirst(Activity smartDetailActivity, FrameLayout loadingweb) {
        String playUrl = mAssist.getReceiverGroup().getGroupValue().getString(DataInter.Key.KEY_CURRENTPLAY_URL);
        if (TextUtils.isEmpty(playUrl)) {
            Toast.makeText(smartDetailActivity, "初始化出错，请退出重试", Toast.LENGTH_SHORT).show();
            return;
        }
        if (playParseUrl(smartDetailActivity, loadingweb, playUrl)) return;
        if (mAssist != null) {
            mAssist.play();
        }
    ...
    private boolean playParseUrl(Activity smartDetailActivity, FrameLayout loadingweb, String playUrl) {
        if (playUrl.endsWith(".html")) {
